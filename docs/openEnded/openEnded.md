...
在好大夫工作期间，主要是负责公司重点、难点、战略型项目的开发和管理工作，包括好大夫的 VIP 项目、好大夫全家宝、预问诊流程、HIS 系统等项目。
...
同时这个项目同时也特别大。前端这边是6个人，我是作为前端负责人的角色。    
- 这个项目的难点主要有这些：
	- 1、开发时间比较紧，任务多。我这边是这样处理的，先把项目分为了不同的模块，比如医生端小程序、患者端小程序和H5、PC 等多个部分。
	...
	- 3、项目对性能要求较高，因为涉及到了所有的主要业务，所以对性能要求很高。我在团队中使用了 lighthouse、webpagetest 对主要页面进行性能优化。
	- 4、项目涉及人员较多，所以在进行 code review 时难免会出现问题。我在团队内容标准化了 eslint 和 sonarlint，对代码整体的风格和质量进行把控。
	- 5、同时还有一个我对业务的思考，如何提升转化率：小程序页面分析系统
- 最后项目结果是：
...
## 2020年前端最火的技术是什么？
我们知道，目前前端工程化能力已经处于比较成熟的阶段，但是提高开发效率的问题还有很远的路要走。     
那么最重要的一点就是将前端人为工作提升到自动化阶段，而自动化必然离不开的就是 AI 。     
前端如何加持 AI？比如可以使用设计稿生成代码(imgcook)。      
